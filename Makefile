...
TRIVY_ARM64_DOWNLOAD_URL=https://github.com/aquasecurity/trivy/releases/download/$(TRIVYVERSION)/trivy_$(TRIVYVERSION:v%=%)_Linux-ARM64.tar.gz
...
TRIVY_ARM64_ADAPTER_DOWNLOAD_URL=https://github.com/aquasecurity/harbor-scanner-trivy/releases/download/$(TRIVYADAPTERVERSION)/harbor-scanner-trivy_$(TRIVYADAPTERVERSION:v%=%)_Linux_arm64.tar.gz
...
DOCKERBUILD=$(DOCKERCMD) build --progress plain
...
compile_core: gen_apis
	...
	@for arch in amd64 arm64; do \
		$(DOCKERCMD) run --rm -v $(BUILDPATH):$(GOBUILDPATHINCONTAINER) -w $(GOBUILDPATH_CORE) -e GOARCH=$$arch $(GOBUILDIMAGE) $(GOIMAGEBUILD_CORE) -o $(GOBUILDPATHINCONTAINER)/$(GOBUILDMAKEPATH_CORE)/$$arch/$(CORE_BINARYNAME); \
	done
	@echo "Done."
...
compile_jobservice:
	...
	@for arch in amd64 arm64; do \
		$(DOCKERCMD) run --rm -v $(BUILDPATH):$(GOBUILDPATHINCONTAINER) -w $(GOBUILDPATH_JOBSERVICE) -e GOARCH=$$arch $(GOBUILDIMAGE) $(GOIMAGEBUILD_COMMON) -o $(GOBUILDPATHINCONTAINER)/$(GOBUILDMAKEPATH_JOBSERVICE)/$$arch/$(JOBSERVICEBINARYNAME); \
	done
	@echo "Done."
...
compile_registryctl:
	...
	@for arch in amd64 arm64; do \
		$(DOCKERCMD) run --rm -v $(BUILDPATH):$(GOBUILDPATHINCONTAINER) -w $(GOBUILDPATH_REGISTRYCTL) -e GOARCH=$$arch $(GOBUILDIMAGE) $(GOIMAGEBUILD_COMMON) -o $(GOBUILDPATHINCONTAINER)/$(GOBUILDMAKEPATH_REGISTRYCTL)/$$arch/$(REGISTRYCTLBINARYNAME); \
	done
	@echo "Done."
...
# PULL_BASE_FROM_DOCKERHUB should be true if BUILD_BASE is not true
	...
	make -f $(MAKEFILEPATH_PHOTON)/Makefile $(BUILDTARGET) -e GOBUILDIMAGE=$(GOBUILDIMAGE) \
	 -e REGISTRY_SRC_TAG=$(REGISTRY_SRC_TAG) \
	 ...
	 -e TRIVY_DOWNLOAD_URL=$(TRIVY_DOWNLOAD_URL) -e TRIVY_ARM64_DOWNLOAD_URL=$(TRIVY_ARM64_DOWNLOAD_URL) \
	 -e TRIVY_ADAPTER_DOWNLOAD_URL=$(TRIVY_ADAPTER_DOWNLOAD_URL) -e TRIVY_ARM64_ADAPTER_DOWNLOAD_URL=$(TRIVY_ARM64_ADAPTER_DOWNLOAD_URL) \
