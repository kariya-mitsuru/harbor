...
TRIVY_ADAPTER_DOWNLOAD_URL=https://github.com/aquasecurity/harbor-scanner-trivy/releases/download/$(TRIVYADAPTERVERSION)/harbor-scanner-trivy_$(TRIVYADAPTERVERSION:v%=%)_Linux_ARCH.tar.gz
...
compile_core: gen_apis
	...
	@for arch in amd64 arm64; do \
		$(DOCKERCMD) run --rm -v $(BUILDPATH):$(GOBUILDPATHINCONTAINER) -w $(GOBUILDPATH_CORE) -e GOARCH=$$arch $(GOBUILDIMAGE) $(GOIMAGEBUILD_CORE) -o $(GOBUILDPATHINCONTAINER)/$(GOBUILDMAKEPATH_CORE)/binary/$$arch/$(CORE_BINARYNAME); \
	done
	@echo "Done."
...
compile_jobservice:
	...
	@for arch in amd64 arm64; do \
		$(DOCKERCMD) run --rm -v $(BUILDPATH):$(GOBUILDPATHINCONTAINER) -w $(GOBUILDPATH_JOBSERVICE) -e GOARCH=$$arch $(GOBUILDIMAGE) $(GOIMAGEBUILD_COMMON) -o $(GOBUILDPATHINCONTAINER)/$(GOBUILDMAKEPATH_JOBSERVICE)/binary/$$arch/$(JOBSERVICEBINARYNAME); \
	done
	@echo "Done."
...
compile_registryctl:
	...
	@for arch in amd64 arm64; do \
		$(DOCKERCMD) run --rm -v $(BUILDPATH):$(GOBUILDPATHINCONTAINER) -w $(GOBUILDPATH_REGISTRYCTL) -e GOARCH=$$arch $(GOBUILDIMAGE) $(GOIMAGEBUILD_COMMON) -o $(GOBUILDPATHINCONTAINER)/$(GOBUILDMAKEPATH_REGISTRYCTL)/binary/$$arch/$(REGISTRYCTLBINARYNAME); \
	done
	@echo "Done."
...
compile_standalone_db_migrator:
	...
	@for arch in amd64 arm64; do \
		$(DOCKERCMD) run --rm -v $(BUILDPATH):$(GOBUILDPATHINCONTAINER) -w $(GOBUILDPATH_STANDALONE_DB_MIGRATOR) -e GOARCH=$$arch $(GOBUILDIMAGE) $(GOIMAGEBUILD_COMMON) -o $(GOBUILDPATHINCONTAINER)/$(GOBUILDMAKEPATH_STANDALONE_DB_MIGRATOR)/binary/$$arch/$(STANDALONE_DB_MIGRATOR_BINARYNAME); \
	done
	@echo "Done."
...
cleanbinary:
	...
	if [ -f $(STANDALONE_DB_MIGRATOR_BINARYPATH)/$(STANDALONE_DB_MIGRATOR_BINARYNAME) ] ; then rm $(STANDALONE_DB_MIGRATOR_BINARYPATH)/$(STANDALONE_DB_MIGRATOR_BINARYNAME) ; fi
