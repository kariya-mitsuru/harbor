...
for arch in amd64 arm64; do
	...
	docker build -f $TEMP/Dockerfile.binary -t trivy-adapter-golang --build-arg GOARCH=$arch $TEMP

	echo "Copying Trivy adapter binary from the container to the local directory..."
	ID=$(docker create trivy-adapter-golang)
	docker cp $ID:/go/src/github.com/aquasecurity/harbor-scanner-trivy/scanner-trivy binary/$arch
